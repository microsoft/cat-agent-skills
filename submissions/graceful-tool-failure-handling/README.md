...
This same discipline already shows up domain-specifically elsewhere: an IT
support intake skill that never invents a ticket number a connector didn't
return, or a company-memory skill that says plainly when no persistent
storage is configured rather than pretending memory will carry over. This
skill generalizes that same honesty to any tool call, for agents that don't
