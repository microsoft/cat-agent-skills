...
def generate_docx(spec,path):
    ...
    d.add_heading('Controls and Evidence',level=1);add_table(d,['ID','Control','Objective','Owner','Frequency','Evidence','Type'],[[c.get(k) or 'TBD' for k in ['id','name','objective','owner','frequency','evidence','type']] for c in spec.get('controls',[])])
...
def generate_xlsx(spec,path):
    ...
    for ctl in spec.get('controls',[]):
        related=[s.get('id') for s in spec.get('steps',[]) if ctl.get('id') in (s.get('control_ids') or [])]
