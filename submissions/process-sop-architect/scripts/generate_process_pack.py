...
def generate_svg(spec,path):
    ...
    lane_x={o:margin+i*lane_w for i,o in enumerate(owners)};pos={}
    ...
    for s in steps:
        ...
        for label,t in targets:
            ...
            parts.append(f'<path d="M {x1} {y1} C {x1} {y1+25}, {x2} {y2-25}, {x2} {y2}" fill="none" stroke="#5B6573" stroke-width="2" marker-end="url(#arrow)"/>')
...
def generate_docx(spec,path):
    ...
    p=d.add_paragraph();p.alignment=WD_ALIGN_PARAGRAPH.CENTER;r=p.add_run(spec['process_name']);r.bold=True;r.font.size=Pt(22);r.font.color.rgb=None
    ...
    d.add_heading('Scope',level=1);sc=spec.get('scope',{});d.add_paragraph(f"Starts: {sc.get('starts','TBD')}");d.add_paragraph(f"Ends: {sc.get('ends','TBD')}");d.add_paragraph('Exclusions: '+(', '.join(sc.get('exclusions',[])) or 'None stated'))
    d.add_heading('Inputs and Outputs',level=1);add_table(d,['Inputs','Outputs'],[[', '.join(spec.get('inputs',[])),', '.join(spec.get('outputs',[]))]])
    d.add_heading('Roles and Responsibilities',level=1);add_table(d,['Role','Responsibility'],[[r.get('name'),r.get('description')] for r in spec.get('roles',[])])
    ...
    for s in spec.get('steps',[]):
        ...
        rows.append([s.get('id'),s.get('name'),s.get('owner'),s.get('description'),s.get('system',''),s.get('sla',''),nxt,', '.join(s.get('control_ids',[]))])
    add_table(d,['ID','Step','Owner','Instructions','System','SLA','Next','Controls'],rows)
    d.add_heading('Controls and Evidence',level=1);add_table(d,['ID','Control','Objective','Owner','Frequency','Evidence','Type'],[[c.get(k,'') for k in ['id','name','objective','owner','frequency','evidence','type']] for c in spec.get('controls',[])])
    d.add_heading('Risks',level=1);add_table(d,['ID','Risk','Impact','Mitigation'],[[r.get('id'),r.get('description'),r.get('impact'),r.get('mitigation')] for r in spec.get('risks',[])])
    d.add_heading('Metrics',level=1);add_table(d,['Metric','Definition','Target','Owner'],[[m.get('name'),m.get('definition'),m.get('target'),m.get('owner')] for m in spec.get('metrics',[])])
    d.add_heading('Assumptions and Open Questions',level=1)
    for x in spec.get('assumptions',[]):d.add_paragraph('Assumption: '+x,style='List Bullet')
    for x in spec.get('open_questions',[]):d.add_paragraph('Open question: '+x,style='List Bullet')
    d.save(path)

def generate_xlsx(spec,path):
    wb=Workbook();r=wb.active;r.title='RACI';roles=[x.get('name') for x in spec.get('roles',[])];r.append(['Step ID','Process Step']+roles)
    for s in spec.get('steps',[]):
        row=[s.get('id'),s.get('name')]
        for role in roles:row.append('R/A' if role==s.get('owner') else 'C' if role in s.get('consulted_roles',[]) else 'I' if role in s.get('informed_roles',[]) else '')
        r.append(row)
    ...
    for ctl in spec.get('controls',[]):
        related=[s.get('id') for s in spec.get('steps',[]) if ctl.get('id') in s.get('control_ids',[])]
        c.append([ctl.get('id'),ctl.get('name'),ctl.get('objective'),ctl.get('owner'),ctl.get('frequency'),ctl.get('evidence'),ctl.get('type'),', '.join(related)])
    header(c);autofit(c)
    m=wb.create_sheet('Metrics');m.append(['Metric','Definition','Target','Owner'])
    for x in spec.get('metrics',[]):m.append([x.get('name'),x.get('definition'),x.get('target'),x.get('owner')])
...
def main():
    ...
    if generate_presentation is not None:
    ...
    with (a.output_dir/f'{slug}_improvement_backlog.csv').open('w',encoding='utf-8-sig',newline='') as f:
        fields=['title','problem','recommendation','value','effort','risk','priority','owner'];w=csv.DictWriter(f,fieldnames=fields);w.writeheader();w.writerows([{k:x.get(k,'') for k in fields} for x in spec.get('improvements',[])])
    lines=[f"# {spec['process_name']} — Process Pack",'',f"**Purpose:** {spec.get('purpose','')}",f"**State:** {spec.get('state','current')}",f"**Steps:** {len(spec.get('steps',[]))}",f"**Controls:** {len(spec.get('controls',[]))}",f"**Improvement opportunities:** {len(spec.get('improvements',[]))}",'**Presentation:** Executive process briefing generated as editable PowerPoint.','','## Open questions']+[f'- {x}' for x in spec.get('open_questions',[])]+['','## Validation warnings']+[f'- {x}' for x in warnings]
