...
The lock is a **directory** (creation is atomic, so two runs starting together cannot both acquire it) with an `owner` file holding the PID and timestamp. On contention the script checks **liveness, not just age**: a lock whose owner process is still alive is respected (a long cold-profile run is legitimate), while a lock whose owner is dead, or older than a 30-minute backstop, is overridden. If the script prints `RUN_ALREADY_ACTIVE`, exit without touching the browser and **do not release the lock** - it belongs to the live run.

The script clears a leftover browser **only when it recovered a stale lock** (a prior run that crashed with the browser still open); it prints a line saying so. On a **clean** acquire it leaves the browser alone, so the existing WhatsApp Web session is reused and there is no close/reopen flicker every run. When it does clear, it targets only Playwright-launched browser binaries (bundled Chromium or the `msedge` channel), never the Node driver or the user's normal browser windows.
...
- Always release the lock at the end of a run that took it, and **only if you own it** (the `owner` file's PID equals this run's): remove the `scout-whatsapp.lock` directory. On POSIX: `[ "$(head -n1 /tmp/scout-whatsapp.lock/owner 2>/dev/null)" = "$$" ] && rm -rf /tmp/scout-whatsapp.lock`. On Windows: check `%TEMP%\scout-whatsapp.lock\owner` holds this PID before `Remove-Item -Recurse`. Never remove it when you exited on `RUN_ALREADY_ACTIVE` - that lock belongs to another run.
