...
Between runs the skill **reuses the already-open WhatsApp session** instead of closing and reopening the browser, so you should not see a close/reopen flicker on each request. It kills and reopens the browser only when a previous run crashed (a recovered stale lock) or the page is genuinely stuck mid-run. When that happens it closes **every** browser running on a Playwright profile for your user, not only the one holding WhatsApp - so another Playwright session Scout is driving at that moment is closed too. Your own browser windows are never touched, and nothing is lost: the profile is persistent, so WhatsApp stays logged in and simply reopens. (If a window still flickers on every run, that is Scout relaunching the browser tool itself, which is a Scout browser setting, not something this skill triggers.)
