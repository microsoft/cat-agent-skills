...
4. **Grouped match** (when `enableGrouped`). Detect one-to-many and many-to-one: a set of up to `groupedMaxMembers` records on one side whose amounts sum, within tolerance, to a single record on the other side (e.g., one invoice settled by three partial payments). Grouped matches go to **Needs Review** with all members listed - splits are common and legitimate, but they should be seen by a human.
5. **Timing difference** (when `enableTimingDetection` and a `timingKeyColumn` is set). Among records still unmatched, pair an A record with a B record that shares the same identity minus the period (the "reduced key") and the same amount within tolerance, but a **different period**. This is the same item posted to a different month - the most common false one-sided break in period reconciliations. Timing pairs go to **Needs Review** with both periods shown. See `references/methodology.md` for the reduced-key rule and a worked example.
6. **Unmatched.** Whatever remains: records only in A → **Unmatched (A)**; records only in B → **Unmatched (B)**. These are shown in the report as **"Only in &lt;source name&gt;"** rather than "A"/"B".
