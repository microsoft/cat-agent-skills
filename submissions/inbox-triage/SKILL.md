...
5. **Move via the bound "move email" capability.** Bind this capability now by inspecting the tools available in the current session - typical names are `workiq_move_email` on Scout and `m365_move_email` on Cowork; use whichever concrete tool the session exposes. If neither is present (or an equivalent by another name), stop the bucket and report - do not guess a tool name and do not proceed with a plan that cannot execute. Pass the resolved folder ID as `destination`. Execute one bucket to completion before starting the next; do not parallelise moves across buckets. If the tool supports only one message per call in the current build, move serially and report progress ("moved 50 of 312").
