...
**Sent, for the active-thread test.** One additional list-emails call on the Sent folder over the active-thread window (`activeThreadWindowDays`, default 14). Pull `id`, `conversationId`, To/Cc recipient addresses, and sent time. Do not pull bodies. You use this to answer: "has the user emailed anyone at this address recently?"
...
- **Unread and recent.** Message is unread AND received within `protection.unreadRecentProtectionDays` (default 3 days). The one narrow exception: a message may still be classified as `notifications` if its sender local part matches a **bounded** automated no-reply token (`noreply`, `no-reply`, `donotreply`, `do-not-reply`, `notifications`, `alerts`, `automated`, `system`, `bot`, ... - equal to the token or the token immediately followed by a separator/digit, so `botany@`/`systematic@` do not match; see `references/classification-rules.md`). Newsletters never bypass this rule - a newsletter you haven't read yet is not stale enough to triage.
...
| `notifications` | Sender local part matches a **bounded** automation token (equal to, or token-plus-separator/digit; see `references/classification-rules.md` - so `botany@`/`systematic@` do not match): `noreply`, `no-reply`, `notifications`, `alerts`, `donotreply`, `automated`, `system`, `robot`, `bot`, `jenkins`, `ci`, `deploy`. Or sender is a known automation platform (Jira, Azure DevOps, GitHub, GitLab, ServiceNow, PagerDuty, Datadog, Snyk, Dependabot, ...). | `folders.notifications` (default `Inbox Triage/Notifications`) |
...
5. **Re-verify protection state immediately before moving each message.** The plan was built from a Step 1 snapshot; time passes before the user approves, so a message can have become protected in between. First, **re-run the Sent-window listing** (the same Sent-folder "list emails" call from Step 1, over `activeThreadWindowDays`) so newly active threads are observed - the active-thread rule depends on that listing, and a message the user *sent* after the plan was built cannot be seen by re-fetching the inbound message alone. Then, for every approved message ID, re-fetch its current state (via the bound "get email" capability, or a fresh targeted "list emails" over the approved IDs) and **skip the move** if any protection now applies: it is flagged (`flag.flagStatus = flagged`), it is now unread and received within the unread-recent window (3 days), it gained a Confidential-or-above sensitivity label, its sender or a To/Cc recipient is now in scope of the org-chart or active-thread protection (using the refreshed Sent-window listing), or it is otherwise newly in scope of the Step 2 protection layer. Report skipped messages ("held back 3 that became active/flagged since the plan"). This keeps the Step 2 guarantee true at execution time, not just at planning time. If the re-fetch or re-listing capability is unavailable, say so and ask the user to confirm before moving on stale state.
